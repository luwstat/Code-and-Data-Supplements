...
hzd <- ff/(1 + odds)
sur <- 1/(1 + odds)
plot(tgrids,hzd,ylab = "h(t)",xlab = "t",type = "l")
plot(tgrids,odds,ylab = "Baseline odds",xlab = "t",type = "l")
plot(tgrids,sur,ylab = "S(t)",xlab = "t",type = "l")

as.data.frame(cbind(bt, se_theta, 
                    lb,ub,zvalue,pvalue), row.names = colnames(D[,3]))

######################################################################################

epsl <- log(as.vector(t(as.matrix(gama)) %*% Il)) + iv %*% 
  bt
epsr <- log(as.vector(t(as.matrix(gama)) %*% Ir)) + iv %*% 
  bt
library(icenReg)
estR <- ic_np(cbind(epsl, epsr))
plot(estR, ylab = "Survival function", 
     xlab = "v",lty = 1)
curve(1 - plogis(x, 0, 1), add = TRUE, col = "red",lty = 3)
legend(1, 0.99, legend = c("S(v)", 
                           "S(v)_hat"), col = c("red", "black"), lty = c(3,1))
A <- getSCurves(estR)
jps <- c(A$Tbull_ints[, 1], A$Tbull_ints[, 2])
st1 <- A$S_curves$baseline[findInterval(jps, A$Tbull_ints[, 
                                                          1])]
st2 <- A$S_curves$baseline[findInterval(jps, A$Tbull_ints[, 
                                                          1]) - 1]
...
if(n == 50){
  goodfit <- (STATISTIC < 0.18845)
}else if(n > 50){
  goodfit <- (STATISTIC < 1.35810/sqrt(n))
}else{
  print("n < 50")
}

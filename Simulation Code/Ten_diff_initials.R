...
################################################################################
#  Run po_fit() with 10 different initial_reg vectors,                        #
...
## 1. Prepare the ten sets of initial values ----------------------------------
initial_reg_list <- list(
  c( 10,  10),
  c(  8,   8),
  c(  5,   5),
  c(  1,   1),
  ...
  c( -5,  -5),
  c( -8,  -8),
  c(10, -10)    # duplicate on purpose; keep or change as you like
)

## 2. Containers for results ---------------------------------------------------
results        <- vector("list", length(initial_reg_list))
...
## 3. Fit the models -----------------------------------------------------------
for (i in seq_along(initial_reg_list)) {
  cat("Fitting model", i, "with initial_reg =", initial_reg_list[[i]], "\n")
  ...
  fit <- tryCatch(
    po_fit(
      ...
      initial_reg   = initial_reg_list[[i]],
      order         = 3,
      nknot         = 9,
  ...
  if (is.null(fit)) next                       # skip remainder if fit failed

...
files <- list.files(path = "C:/Users/LW HomeOffice/Dropbox/projectLeftTruncation/simulation/data",pattern = 'rtem_rdm.........txt|rtem_rdm........txt|rtem_rdm.......txt') 
...
for(j in files) {
  ...
  mydata <- read.table(paste('C:/Users/LW HomeOffice/Dropbox/projectLeftTruncation/simulation/data',data_name,sep = "/"), header = TRUE, sep = "\t")
  rt <- read.table(j, header=TRUE)
  #real beata
  realbt <- c(round(mean(rt[complete.cases(rt),1])),round(mean(rt[complete.cases(rt),2])))
  #time points
  t <- seq(0,6,0.05)
  #the value of CDF for every t
  bslodds <- matrix(rep(NA,length(t)*500),500,length(t))
  
  for(i in which(complete.cases(rt))){
  ...
  odds <- apply(bslodds[complete.cases(bslodds),],2,mean)
  ...
  mse <- apply((t(bslodds[complete.cases(bslodds),]/(1+bslodds[complete.cases(bslodds),])) - bsl/(1+bsl))^2,1,mean)
  #check baseline odds
...
write.csv(newobject2,file = 'C:/Users/LW HomeOffice/Dropbox/projectLeftTruncation/simulation/data/result_rdm.csv')
